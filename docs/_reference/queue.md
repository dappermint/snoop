...
8. **Changes appear immediately.** Snoop updates the queue before Spotify
   confirms the change. For local playback, it updates its own player directly.

9. **Closing the app keeps the queue.** Snoop saves it locally. When you
...
10. **Old answers from Spotify are ignored.** Queue responses can be a few
    seconds late. Snoop ignores stale responses and asks again. Your
