...
/// happens to hold the port is never mistaken for Snoop.
#[cfg(not(target_os = "linux"))]
const PREFIX: &str = "snoop:";
#[cfg(not(target_os = "linux"))]
const OK_REPLY: &str = "snoop:ok";
#[cfg(not(target_os = "linux"))]
const NOW_REPLY: &str = "snoop:now ";
#[cfg(not(target_os = "linux"))]
const DEVICES_REPLY: &str = "snoop:devices ";
...
fn send_to(port: u16, verb: &str) -> std::io::Result<Reply> {
    ...
    if line == OK_REPLY {
        ...
        Err(std::io::Error::new(
            ...
            "the port is held by something other than Snoop",
        ))
    }
}
...
mod tests {
    ...
    fn parses_every_control_verb() {
        // #given / #when / #then
        assert_eq!(command("snoop:show\n"), Some(ControlCommand::Show));
        assert_eq!(command("snoop:playpause"), Some(ControlCommand::PlayPause));
        assert_eq!(command("snoop:play"), Some(ControlCommand::Play));
        assert_eq!(command("snoop:pause"), Some(ControlCommand::Pause));
        assert_eq!(command("snoop:next"), Some(ControlCommand::Next));
        assert_eq!(command("snoop:previous"), Some(ControlCommand::Previous));
        assert_eq!(
            command("snoop:seek-by -10000"),
            Some(ControlCommand::SeekBy(-10_000))
        );
        assert_eq!(
            command("snoop:volume-by +5"),
            Some(ControlCommand::VolumeBy(5))
        );
        assert_eq!(
            command("snoop:volume-set 40"),
            Some(ControlCommand::SetVolume(40))
        );
        assert_eq!(command("snoop:mute"), Some(ControlCommand::ToggleMute));
        assert_eq!(
            command("snoop:shuffle"),
            Some(ControlCommand::ToggleShuffle)
        );
        assert_eq!(command("snoop:repeat"), Some(ControlCommand::CycleRepeat));
        assert_eq!(
            command("snoop:shuffle-set on"),
            Some(ControlCommand::SetShuffle(true))
        );
        assert_eq!(
            command("snoop:shuffle-set off"),
            Some(ControlCommand::SetShuffle(false))
        );
        assert_eq!(
            command("snoop:repeat-set track"),
            Some(ControlCommand::SetRepeat(RepeatMode::Track))
        );
        assert_eq!(
            command("snoop:repeat-set context"),
            Some(ControlCommand::SetRepeat(RepeatMode::Context))
        );
        assert_eq!(
            command("snoop:repeat-set off"),
            Some(ControlCommand::SetRepeat(RepeatMode::Off))
        );
        assert_eq!(
            command("snoop:seek-to 90000"),
            Some(ControlCommand::SeekTo(90_000))
        );
        assert_eq!(
            command("snoop:save-toggle"),
            Some(ControlCommand::ToggleSaved)
        );
        assert_eq!(
            command("snoop:play-uri spotify:playlist:37i9dQZF1DXcBWIGoYBM5M"),
    ...
    fn rejects_lines_that_are_not_ours() {
        ...
        assert!(parse("snoop:frobnicate").is_none());
        assert!(parse("snoop:seek-by soon").is_none());
        assert!(parse("snoop:volume-set 999").is_none());
        assert!(parse("snoop:next please").is_none());
    ...
    fn refuses_arguments_that_are_not_shaped_like_spotifys_own() {
        // #given / #when / #then
        assert!(command("snoop:play-uri http://example.com/pwn").is_none());
        assert!(command("snoop:play-uri spotify:track:a b").is_none());
        assert!(command("snoop:play-uri ../../etc/passwd").is_none());
        assert!(command("snoop:play-uri").is_none());
        ...
        assert!(command("snoop:shuffle-set maybe").is_none());
