...
impl App {
                    ...
                    match result {
                        Ok(Some(notice)) => {
                            if manual || self.update.as_ref() != Some(&notice) {
                                self.toast(format!("Snoop {} is available", notice.version));
                            }
                        ...
                        Ok(None) => {
                            self.update = None;
                            if manual {
                                self.toast("Snoop is up to date");
                            } else {
    ...
    fn open_pending_link(&mut self) {
        ...
        match util::uri_kind(&uri) {
            ...
            _ => {
                self.pending_link = None;
                self.toast_error("Snoop cannot open this kind of Spotify link");
            }
        }
    }
...
impl App {
    ...
    fn sync_window_title(&mut self, ctx: &egui::Context) {
        let title = match self.now_playing().filter(|now| now.playing) {
            Some(now) if now.subtitle.is_empty() => format!("{} - Snoop", now.title),
